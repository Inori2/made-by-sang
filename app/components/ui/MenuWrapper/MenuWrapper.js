...
export default function MenuWrapper({ isOpen, onLinkClick }) {
  ...
  // Reveal/hide animation — synced with navbar (duration: 1, ease: power2.inOut)
  useEffect(() => {
    if (!wrapperRef.current) return;

    // Start: zero width, zero height, hidden to prevent flash
  ...
  useEffect(() => {
    ...
    if (!isOpen) {
      // Opening: wait for navbar to finish expanding (1s) then play
      delayedCallRef.current = gsap.delayedCall(0.1, () => tlReveal.current.play());
    } else {
      // Closing: reverse immediately
...
useEffect(() => {
  ...
  return (
    ...
    >
      <div className={style.menuLeft} ref={menuLeftRef} style={{ transform: "translateY(50px)" }}>
      ...
      <div className={style.menuCenter} ref={menuCenterRef} style={{ transform: "translateY(50px)" }}>
        
      </div>
      <div className={style.menuRight} ref={menuRightRef} style={{ transform: "translateY(50px)" }}>
